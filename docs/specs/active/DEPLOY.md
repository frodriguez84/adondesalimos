...
| 4 | **Base: Neon Free, región `aws-sa-east-1` (São Paulo); funciones de Vercel en `gru1` (São Paulo).** ⚠️ **La región vive en `vercel.json` (`"regions": ["gru1"]`), no en el panel** (2026-08-07): el default de Vercel para proyectos nuevos es `iad1` y **el setting del dashboard no llegó al deployment** — se vio en el header `x-vercel-id`, que devolvía `gru1::iad1::…` (entra por São Paulo, ejecuta en Virginia). En el repo la región es verificable leyendo el código; en el panel es invisible. Los dos en Sudamérica: ~30 ms desde AR contra ~120 ms si la base quedara en Virginia, y el motor de búsqueda hace varias queries por página. Mover un proyecto de Neon de región después es dump/restore, así que se elige bien de entrada. |
...
| 8 | **`ai.chat_monthly_cap` baja de 5.000 a 500 para el lanzamiento** — un `UPDATE` en `app_settings`, cero código, cero deploy. Techo duro de ~US$20/mes. Al llegar, `reservarCupo` corta **antes** de llamar a Anthropic y el usuario ve un banner rioplatense ("El chat está descansando un rato / Volvé más tarde y seguimos") con el input bloqueado, sin perder su mensaje: es la degradación que diseñó la decisión 15 de CHAT_IA, ya probada. Se sube cuando el consumo real se vea en `/admin`. **Medido el 2026-08-07 con los primeros 7 mensajes reales: US$ 0,15 ⇒ ~US$ 0,021 por mensaje ⇒ el cap de 500 techa en ~US$ 11/mes, no en 20.** El equivalente real a US$ 20 serían ~930 mensajes. No se toca ahora (no hay usuarios), pero cuando haga falta subirlo **el número está medido, no estimado**. El caché explica el costo: 67.550 tokens de lectura de caché contra 7.536 de entrada — **9×** — que es el prefijo de 8.776 tokens releído en cada turno y cobrado a 0,1×. |
...
| 22 | **Hay canal de contacto, y es `contacto@adondesalimos.com.ar` por Cloudflare Email Routing — gratis, sin código, y se hace en la misma visita al panel de DNS que Vercel.** Decidido con Fer el 2026-08-03, al escribir el aviso de beta: el copy invitaba a "avisanos" y **no había dónde** (Resend está configurado solo para enviar; `no-reply@` no recibe, y quien conteste ese mail escribe al vacío). El apex **no tiene MX** —los de Resend cuelgan de `send.`, decisión 19— así que está libre y **no hay conflicto**: se agregan los 3 MX de Cloudflare y su TXT de SPF **en el apex**, sin tocar nada de `send.*` ni de `resend._domainkey.*`. ⚠️ **Es solo reenvío**: llega a la casilla personal de Fer, pero al responder sale desde su dirección, no desde `contacto@`. Aceptable en beta con tráfico ralo. **El timing es el argumento**, no la feature: en F1 hay que entrar igual a Cloudflare para apuntar Vercel, y hacerlo ahí cuesta cinco minutos más. **Se descartó el formulario in-app** (tabla + endpoint + tab de `/admin` + qué hacer con el spam): sin usuarios es infraestructura para cero mensajes, y cuando el volumen moleste se va a saber además **qué tipo** de mensajes llegan, que es lo que hoy no se sabe y define cómo se diseña. Bonus: la promesa de la decisión 6 (*"te escribimos apenas abramos los pagos"*) pasa a cumplirse **desde el dominio** en vez de desde un Gmail personal — menos spam y menos cara de phishing. **La dirección cambió de `hola@` a `contacto@` el 2026-08-07**, al ejecutar F1: `hola@` estaba elegida por voz de producto, pero la misma casilla la va a leer un dueño de restaurante cuando arranque el B2B (spec 7), y ahí lo informal juega en contra. `contacto@` es la convención en AR y no obliga a nadie a deducir para qué es. Email Routing admite varias direcciones a la misma casilla sin costo, así que esto es **cuál se publica**, no cuál existe. |
...
> buscaste y no apareció: contanos a **contacto@adondesalimos.com.ar**. En esta etapa cada mensaje
> nos sirve muchísimo.
>
...
1. ~~No hay canal de contacto.~~ **Resuelto: decisión 22** — `contacto@adondesalimos.com.ar` por
...
- [x] El chat contesta, descuenta cupo, y el tablero de `/admin` muestra el costo con los tokens de caché. ✅ DEPLOY-09 (3→2→1→0 con el gate) + tablero verificado el 2026-08-07: la columna es **`TOKENS (IN / OUT / CACHÉ)`** y muestra **67.550 de caché** aparte de los 7.536 de entrada — el caché **está contado**.
...
| DEPLOY-21 | Mail de prueba a `contacto@adondesalimos.com.ar` | Llega a la casilla de Fer. **Y en la misma corrida**: un registro nuevo sigue recibiendo su mail de verificación (que los MX del apex no rompieron a Resend) |
