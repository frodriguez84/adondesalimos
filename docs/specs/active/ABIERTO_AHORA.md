...
**Estado:** Parcial — **F1 ✅ Implementado (2026-07-30)**; **F2 pendiente**, gateada (decisión 11)
...
### Lo que quedó implementado (2026-07-30)

- `lib/search/ahora.ts` — `FRANJAS` + `franjaActual(now)` (reusa `partesEnAR`) y los dos
  constantes del chip: `SLUG_AHORA = 'para-ahora'`, `NOMBRE_AHORA = 'Para ahora'`. El rótulo es
  una constante del módulo en vez de un campo por franja (decisión 2: es el mismo en las cinco).
- `lib/search/chips.ts` — `getOccasionChips(now = new Date())` antepone el chip a `home` si su
  `countPlaces` da > 0. El conteo arranca **en paralelo** con los de los chips de Ocasión, para
  no sumarle un round-trip en serie al render de la home.
- **El chip no descuenta de los 4 de la decisión 6 de BUSQUEDA**: la home queda en 1 + 4. Lo
  contrario sacaría un chip de Ocasión de la home a ciertas horas — una regresión silenciosa a
  cambio de nada. `CHIPS_EN_HOME` sigue contando chips **de Ocasión**.
- **UPDATE aplicado** el 2026-07-30 sobre el Postgres de dev (con `npm run backup:db` antes):
...
- [x] `franjaActual` cubre las 24 h sin huecos ni solapamientos, en TZ AR, y es puro respecto de
      `now` (test con `Date` fijo, sin depender del reloj de la máquina).
- [x] La madrugada (`00:00–05:59`) devuelve **los dos** tags (`trasnoche`, `hasta-tarde`).
...
- [x] `lib/search/query.ts`, `lib/search/params.ts` y `components/search/occasion-chips.tsx` no
      tienen cambios.
- [x] `select active from tags where slug = 'abierto-ahora'` = `false`; el tag no aparece en el
      sheet de filtros ni en las cards, y sus 20 filas de `place_tags` siguen existiendo.
- [x] typecheck + tests + build verdes. _(typecheck ✅ · tests ✅ 497/497 · build ✅ con el dev
      server parado)_
