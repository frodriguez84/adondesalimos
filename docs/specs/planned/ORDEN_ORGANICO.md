...
- [ ] `lib/search/cadenas.ts` existe, es el **único** módulo que lee `search.cadenas`, valida el
      ...
      el script generador — **ningún consumidor más**
- [ ] `clavesDeOrden` incluye la banda con la precedencia de la decisión 3, dentro del
      `if (!usaGps)` y entre `ownerRank` y `confKey`; test unitario de las 4 bandas
- [ ] **El cursor sobrevive**: test de integración que pagina las 3 primeras páginas de una zona y
      verifica cero duplicados y cero saltos contra el resultado completo ordenado
- [ ] **Nada se filtra**: test que compara `countPlaces` antes y después del cambio para una
      matriz de búsquedas (con zona / sin zona / con chip / con texto) y exige **igualdad exacta**
- [ ] Los 4 chips de la home siguen prendidos con los mismos conteos: `npm run cobertura-chips`
      da la misma matriz que hoy (el piso se calcula sobre `countPlaces`, que no cambió)
- [ ] `buscarDestacados` no se toca — `git diff` no muestra cambios en ese bloque
- [ ] `scripts/cadenas.ts` + `npm run cadenas:proponer` imprimen la lista y el `UPDATE`, **sin
      escribir en la base** (verificable: el script no importa nada que escriba)
- [ ] El seed siembra `search.cadenas` con la lista inicial y es idempotente
- [ ] Índice parcial creado por migración y **costo re-medido** con `EXPLAIN ANALYZE` en los dos
      casos (con zona y sin zona), con el número anotado en el spec
- [ ] En modo GPS el orden **no cambia**: test que verifica que la banda no altera el resultado
      cuando hay coordenadas
- [ ] `npx tsc --noEmit` · `npm test` · `npm run build` verdes (el build con el dev server parado)

## QA manual (IDs propuestos)

| ID | Caso | Criterio |
|----|------|----------|
| ORD-01 | Palermo Soho, sin chip | El top 20 no tiene ninguna cadena de la lista; #1 y #2 no son Burger King ni Subway |
| ORD-02 | Palermo Soho + chip **Cenar afuera** | El top 10 es el de la sección *Objetivo*; McDonald's no está en la primera página |
| ORD-03 | Palermo Soho + chip **Un café** | Starbucks **no** está en el top 10 (es el caso que mata la precedencia invertida de la decisión 3) |
| ORD-04 | Quilmes (conurbano), sin chip y con **Cenar afuera** | Mismo criterio que ORD-01/02 con oferta chica: el top sigue lleno y sin cadenas |
| ORD-05 | Las 46 zonas | Ninguna queda con la primera pantalla vacía o más corta que hoy (el orden no filtra) |
| ORD-06 | Scroll de 3 páginas en Palermo Soho | Ninguna card repetida, ninguna salteada; el conteo del sheet coincide con lo que se puede scrollear |
| ORD-07 | "Cerca de mí" (GPS) | El orden es por distancia, idéntico al de hoy: la banda no interviene |
| ORD-08 | Texto libre "burger" | Burger King aparece arriba: con texto manda la similitud y la banda solo desempata |
| ORD-09 | Vista mapa con resultado > 200 | Los pins son los mismos lugares que encabezan la lista (decisión 17) |
| ORD-10 | `UPDATE app_settings SET value='[]' WHERE key='search.cadenas'` | El orden degrada al de hoy sin error y sin pantalla rota; volver a poner la lista lo restaura (decisión 16) |
