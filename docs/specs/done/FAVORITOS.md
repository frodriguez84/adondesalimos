...
**Estado:** ✅ Implementado (2026-07-31) — F1 (2026-07-30) + F2 (2026-07-31).
QA: `docs/qa/AnalisisQA.md` § FAVORITOS F1 y § QA /qa-spec — FAVORITOS F2.
...
| `app/api/favoritos/route.ts` (POST · DELETE · **GET por lote**), `app/api/listas/route.ts`, `app/api/listas/[id]/route.ts` | Adaptadores finos (decisión 14) |
...
**Lo que se decidió al implementar F2** (2026-07-31; el detalle y su evidencia están en el QA):

- **La default ocupa un lugar del cupo aunque todavía no exista** (`listasOcupadas`, en el dueño
  único). Sin eso, un free sin nada guardado podía crear una lista con nombre y quedarse sin destino
  para el tap —o con una default que su propio cupo esconde.
- **Borrar una lista sí borra sus ítems** (cascade) y **no contradice** "ocultar ≠ borrar": ese
  invariante prohíbe borrar por un cambio de plan, no por un pedido explícito del usuario (que ve
  cuántos lugares se van antes de confirmar).
- **El botón de `/mis-lugares` lleva `listId`**: sacar desde una lista saca de **esa**. El de la
  card sigue sacando de todas las visibles, porque ahí el estado es por lugar y no por lista.
- **`estadoDeFavoritos` reemplaza a `guardadosDeLaPagina` en la home y la ficha**: guardados y listas
  salen de la misma resolución de `listasVisibles` (el sheet necesita las dos cosas). `/api/search`
  sigue con `guardadosDeLaPagina` — las listas no cambian entre páginas del scroll.
- **El chat pide el estado por lote** (`GET /api/favoritos?ids=`), una request por tanda de cards
  nuevas: es la única superficie donde las cards llegan por streaming y no hay server que las mire.
...
- [x] Un usuario **free** que intenta crear una segunda lista recibe error **desde el server**:
      `POST /api/listas` → 403 `LIMITE_LISTAS` (FAV-04). El número sigue saliendo del dueño único
      (`puedeCrearLista`), y la default **ocupa su lugar del cupo aunque todavía no exista** —
      si no, un free gastaría su única lista en una con nombre y el tap se quedaría sin destino.
- [x] Un **premium** puede crear hasta `favoritos.max_listas_premium` listas y ni una más
      (FAV-31, test): el conteo va dentro de la transacción con la fila del usuario `FOR UPDATE`.
- [x] Bajar el plan a `free` **no borra ninguna fila**: las listas no-default quedan y dejan de
      ...
      la cuenta.
- [x] La lista default no se puede borrar ni renombrar: las dos acciones cortan con
      `LISTA_DEFAULT` → 403 (FAV-12), y la UI ni siquiera muestra los botones.
...
- [x] Desde la card del listado y desde la ficha, un tap guarda el lugar, directo a la default,
      creándola si es el primer guardado.
- [x] Con **más de una** lista visible, el tap abre el sheet de destino (FAV-05); con una sola
      —todo usuario free— sigue siendo un tap y listo. Anda en las tres superficies con botón:
      card, ficha y chat (FAV-27/28). El sheet reusa `components/ui/bottom-sheet.tsx` y se monta
      solo cuando se abre (si no, cada card dejaría un overlay fijo dormido en el DOM).
...
- [x] Sin sesión redirige a login (307 → `/login?callbackUrl=/mis-lugares`, FAV-20); con sesión
      muestra las listas visibles y sus lugares, más recientes primero.
- [x] Un lugar despublicado sigue apareciendo, atenuado, con "Ya no está disponible" y sin link
      (decisión 11; FAV-10 — la ficha del mismo id da 404).
- [x] Premium puede crear, renombrar y borrar listas no-default desde la página (FAV-22/24/25),
      con confirmación antes de borrar. Sacar un lugar desde acá lo saca de **esa** lista y no de
      todas (FAV-26).
- [x] Link presente en el `AccountMenu`, al lado de "Mis votaciones" (FAV-21).
...
- [x] typecheck + tests verdes (**523/523** al cerrar F2) + build.
