...
| 9 | **Headline rotativo = aleatorio por visita, resuelto client-side** (`components/shared/rotating-headline.tsx`, `'use client'`). SSR renderiza siempre la primera frase (determinista, sin hydration mismatch — lección AUTH F4); un `useEffect` elige una de las tres al azar tras montar. Las tres: **"¿Qué sale?" · "¿Qué pinta?" · "¿Qué hacemos?"** (rioplatense; el "¿Qué se te antoja?" queda descartado, suena a doblaje). |
...
| HOME_IDENTIDAD-04 | Rotación del headline | Recargar varias veces muestra distintas frases; sin warning de hydration en consola. |
