...
**Estado:** ✅ Implementado (2026-07-25) — F1 (Instrumentación + precios), F2 (cobro MP), F3 (destaque) y F4 (desglose), las 4 fases cerradas. QA: `docs/qa/AnalisisQA.md` § MONETIZACION F1, F2, F3 y F4
...
| **4 — Desglose** ✅ | Sección de estadísticas paga en el panel (vistas · impresiones · taps · top filtros · vs mes anterior · destacada X de Y) gateada server-side | Un lugar `paid` ve el desglose con los datos acumulados desde F1; volver a `free` lo devuelve al teaser exacto de AUTH — **implementada 2026-07-25** (`lib/negocio/query.ts` `desgloseEstadisticas` gate por `owner_plan='paid'`, `components/negocio/desglose-panel.tsx`, wire en `app/mi-negocio/[placeId]/page.tsx`; MONE-15, live + test del gate) |
...
- [x] Desglose: con `owner_plan='paid'` el panel muestra vistas · impresiones · taps por tipo ·
      top filtros · vs mes anterior · "destacada en X de Y"; con `free` responde exactamente el
      teaser de AUTH (gate en la query, test) — **F4** (`desgloseEstadisticas`; MONE-15, live + test del gate)
