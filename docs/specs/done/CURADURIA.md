...
**Estado:** ✅ Implementado (2026-07-27) — F1 (batch + migración) ✅ · F2 (cola en `/admin`) ✅ ·
F3 (corrida completa autónoma de las 46 zonas con Sonnet, decisión 13) ✅. Piloto aprobado por
Fer (decisión 11). QA: `docs/qa/AnalisisQA.md § CURADURIA (F1+F2) y § CURADURIA F3`. Cobertura:
5/9 chips objetivo prendidos (46/46 zonas con ≥1 chip); los 4 en 0 = dato base no curable
(decisión 12), anotados en `BACKLOG`.
...
- **No auto-aplica tags** _(amendado por la decisión 13 para la corrida masiva)_. El LLM sugiere;
  en la cola manual, solo la confirmación de Fer escribe `place_tags`. En la corrida completa
  autónoma (decisión 13), las sugerencias **con evidencia** se auto-aplican; las **sin evidencia**
  nunca — quedan `pending`.
...
| 13 | **Corrida completa AUTÓNOMA (Fer, 2026-07-27, tras aprobar el piloto): las sugerencias CON evidencia se auto-aplican a `place_tags` (`source='admin'`) sin confirmación por lugar; las SIN evidencia quedan `pending` en la cola — nunca se auto-aplican, tampoco se descartan.** Fer confía en el criterio del modelo donde hay fuente citable (~97% fiel en el piloto); las adivinanzas por nombre (~46% del piloto) no ensucian el catálogo pero quedan disponibles para revisión opcional. **Modelo de esta corrida: Sonnet** (`ai.curation_model` — swap sin deploy): sin red humana, su mejor razonamiento evita errores tipo "la cita dice *cerrado* pero taggeó `abre-domingos`" (2 casos en el piloto con Haiku). Amend de la decisión 8 **solo** para la corrida masiva; la cola manual sigue disponible e intacta. **Requiere cambio chico en F3:** el batch, además de upsertear en `place_tag_suggestions`, escribe a `place_tags` las que tienen evidencia y las marca `accepted` (reusa el criterio de `guardarCuraduria`); las sin evidencia siguen el camino actual (`pending`). Cubrir con test: con-evidencia → `place_tags` + `accepted`; sin-evidencia → `pending`, `place_tags` sin cambios |
