...
**Estado:** ✅ **Implementado (2026-08-03)** — las 4 fases cerradas. **Único DoD no verificado:
PBETA-07** (el ícono en la pantalla de inicio de **iOS**), por falta de un iPhone a mano; Fer decidió
cerrar igual con el pendiente anotado. El `apple-touch-icon` 180 está servido y linkeado, pero nadie
lo vio en un iPhone. Detalle por fase:
**F1 ✅ 2026-08-03**: los 6 recorridos auditados en vivo a 390×844 (+ control
...
**F4 ✅ 2026-08-03** (`app/manifest.ts` + 4 íconos + `themeColor`; los 8 criterios de instalabilidad
de Chrome medidos en vivo y **la app instalada de verdad en el Android de Fer**, con el splash del
manifest). En el mismo tramo se cubrió el **alta nueva end-to-end**, que era el hueco de F1 y F3: el
arreglo de PBETA-R3-03 **sí sobrevive a una cuenta nueva** en la misma pestaña, y **no** si el link
del mail se abre en otra → hallazgo **PBETA-R3-07** (nuevo, al `BACKLOG`). QA § *PULIDO_BETA F4*.
**Único DoD no verificado: PBETA-07 (iOS)**, por falta de un iPhone a mano.
...
- [x] `app/manifest.ts` existe y la app **se ofrece para instalar** en Android por HTTPS; el ícono
      correcto aparece en la pantalla de inicio en iOS. ✅ F4 en **Android** — Fer la instaló en su
      celular y abre con el splash del manifest (PBETA-06); los 8 criterios de Chrome, medidos en
      vivo. **iOS (PBETA-07) queda sin probar** por falta de un iPhone: el `apple-touch-icon` 180
      está servido y linkeado, pero eso no es lo mismo que haberlo visto
- [x] `theme_color` / `background_color` coinciden con la paleta de `HOME_IDENTIDAD` — no hay
      colores nuevos. ✅ F4 — los dos `#0D0D1F`, que es `--background` de `globals.css`
- [x] **La base quedó como estaba**: lo creado durante la auditoría, borrado o anotado con su `id`
      (decisión 13). ✅ F1 y F3
- [x] typecheck + tests + build verdes (build con el dev server parado). ✅ F4 — `tsc --noEmit`
      limpio, **645 tests en 58 archivos** y `npm run build` *Compiled successfully in 6,6 s* con el
      server parado.
