...
| 13 | **Resultados visibles EN VIVO** para todos, no solo al cerrar: ver el conteo/% por opción mientras está abierta es lo social y lo que empuja a re-compartir ("vamos 2 a 2, voten"). Es el motor del loop viral, no un lujo. Conteo agregado por opción — nunca quién votó qué. ⚠️ **ENMENDADA PARCIALMENTE el 2026-08-14 por [`INVITACION`](INVITACION.md) (decisión 6): el desglose por opción se ve recién con el voto puesto; el total se ve siempre.** El hallazgo `PBETA-R2-12` midió el costo de "para todos": el que llega último ve que uno ya ganó y vota eso, o no vota. Lo que la enmienda **no** toca es a quién le sirve esta decisión — el que re-comparte ya votó, así que conserva los resultados en vivo intactos. Cerrada, vencida o cancelada ⇒ se ve todo, con o sin voto propio (decisión 15, intacta) |
