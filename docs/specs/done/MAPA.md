...
## Ajustes en la implementación (2026-08-08)

Cuatro cosas que el spec decía de otra forma y el código obligó a cambiar. Ninguna toca una
decisión de producto; las cuatro son el **cómo**:

1. **El rótulo se pisa por `locale` del `Map`, no editando el DOM tras `addControl`**
   (decisión 7). MapLibre arma el botón de forma asíncrona —espera a `navigator.permissions`—
   y lo vuelve a rotular en cada cambio de estado: pisar el atributo una vez no aguanta. De
   paso se traduce `GeolocateControl.LocationNotAvailable` → «No podemos ubicarte», que es el
   mismo botón con el permiso ya denegado.
2. **`min-h-80` en vez de `min-h-0 min-h-[20rem]`** (decisión 9). Son la misma propiedad CSS
   peleándose; el piso es el que expresa la intención y `min-h-0` no aportaba nada (el
   contenedor no tiene contenido con alto intrínseco). `min-h-80` = `20rem`, y es la clase
   canónica de Tailwind v4.
3. **El alto del div del mapa sale de `flex-1`, no de `size-full`.** Al sacar el `h-[70vh]`,
   el `height: 100%` del hijo dejó de resolver —necesita un alto **declarado** en el padre— y
   **el mapa colapsaba a 0 px**: el canvas quedaba desbordado en 300 px y los controles no
   recibían el toque. `absolute inset-0` tampoco sirve: el CSS de MapLibre pisa el `position`
   con su propio `.maplibregl-map`. Lo encontró el QA en vivo con los 663 tests en verde.
4. **La fila de chips lleva barra de scroll propia** (`.barra-scroll-marca` en
   `app/globals.css`), no la del sistema ni ninguna. Sin barra, en escritorio no hay de dónde
   agarrar para llegar a los chips de la derecha (con el dedo se arrastra la fila, con el mouse
   no). 6 px, naranja de acción sobre riel del color del borde, sin los triangulitos de Windows.
   **Ojo:** en Chromium ≥ 121 `scrollbar-width`/`scrollbar-color` **anulan** los
   `::-webkit-scrollbar`, así que lo estándar va detrás de `@supports not selector(...)`.

Y un aviso que el spec no definía: los de la § Copy se **auto-ocultan a los 6 s**. Sin eso,
«Por ahora andamos solo por Buenos Aires» quedaba pegado tapando «Cargando el mapa…» el resto
de la visita.
