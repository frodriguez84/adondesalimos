...
**Estado:** ✅ Implementado (2026-07-22) — F1 Auth base (2026-07-20) · F2 Reclamo + alta + cola (2026-07-21) · F3 Panel + contenido (2026-07-21) · F4 Horarios propios (2026-07-22)
...
### Notas de implementación — F4 (2026-07-22)

Lo que el spec no fijaba y quedó decidido al implementar (cierra el spec):

- **Modelo de horarios**: `opening_hours` guarda un objeto `{ [día]: Rango[] }` con los 7 días
  (lunes→domingo), cada `Rango` un `{ abre, cierra }` en `hh:mm`. Un rango cuyo cierre es menor
  que la apertura **cruza la medianoche** y pertenece al día en que abre. Todo vive en el módulo
  puro `lib/negocio/horarios.ts`, con tabla de tests (trasnoche incluido).
- **El cálculo de "abierto ahora" corre en el cliente, tras montar** (`useEffect`), no en el
  server: depende del reloj y calcularlo en el render desincronizaría la hidratación. La semana
  (que no depende de la hora) sí se pinta directo. La TZ es fija `America/Argentina/Buenos_Aires`
  vía `Intl` — no depende del reloj de quien mira.
- **Horarios = free, fuera del gate de plan** (decisión 20). No pasan por `resolverContenidoDueno`
  (ese helper es COALESCE + gating de plan): no hay dato base de Overture con qué hacer COALESCE,
  y la prioridad dueño → Google la resuelve la ficha en el cliente, igual que las fotos.
- **Misma condición `reclamado` que el resto del contenido**: `getPlaceDetail` solo expone
  `horariosDueno` si el lugar tiene reclamo aprobado; revocar lo devuelve a Google sin borrar la
  fila. Una semana entera vacía se persiste como `null` (mismo criterio que `vacioANull`).
- **El field mask de Google no se tocó** (cuidado explícito): el "bonus de costo" de la decisión
  20 es de UI —el bloque de Google pierde los horarios—, pero el rating sigue viniendo de la misma
  request Enterprise, así que sacar `regularOpeningHours`/`currentOpeningHours` del mask no
  ahorraría un peso y rompería tests. La atribución cambia de "Horarios y calificación" a
  "Calificación" cuando el dueño puso sus horarios.
- **Validación server-side de los rangos** (el cliente no es boundary): `hh:mm` bien formado,
  cierre distinto de la apertura, sin solapamientos (proyección a 48 h para cubrir el cruce) y
  tope de 3 franjas por día. El mismo `horariosSchema` valida en el editor y en el `PATCH`.
- **Reconciliación de DoD de F1/F2**: al cerrar el spec entero se tildaron los DoD de registro+
  verificación, Google OAuth, sesión inline, reclamo end-to-end y "Registrá tu negocio" contra el
  QA ya registrado (F2) — no se re-verificaron a mano. Ver QA § AUTH F4.
...
- [x] Registro con email+password crea usuario; el mail de verificación llega (Resend) y
      **sin verificar no hay login** (`requireEmailVerification: true`) — F1, QA AUTH-01/AUTH-02
- [ ] Google OAuth funciona con las env vars presentes; sin ellas el botón no se renderiza
      — **diferido a BACKLOG** (F1, a pedido 2026-07-20): la config de better-auth ya lo soporta
      condicional por env (`lib/auth/index.ts:98`), pero **falta la UI del botón** + exponer el
      flag al cliente. Único DoD no cerrado; deferral aceptado, no regresión
- [x] Todo route handler nuevo (`claims`, `mi-negocio`, `admin`) verifica sesión inline con
      `getSession`; `/admin` + sus endpoints rechazan a cualquier no-admin (`ADMIN_EMAIL`)
      — F2, QA AUTH-14/AUTH-12
- [x] Reclamo end-to-end: usuario verificado reclama desde la ficha → aparece en la cola →
      ...
      — F2, QA AUTH-02/AUTH-03
- [x] "Registrá tu negocio" busca en el catálogo **completo** (visibles e invisibles) antes
      de ofrecer el alta; el alta crea `source='owner'` invisible con zona asignada por la
      geometría de ZONAS; rechazar lo deja invisible — F2, QA AUTH-04/AUTH-05
...
- [x] Horarios propios: la ficha los prioriza sobre Google y el estado abierto/cerrado es
      correcto en TZ AR, incluido un rango que cruza medianoche — F4, QA AUTH-11/11a/11b/11c
...
- [x] Ningún secreto nuevo llega al bundle del browser (R2, Resend, `ADMIN_EMAIL`,
      ...
      5 de R2
- [x] `typecheck` + tests (331/331) + `build` verdes (build con el dev server parado)
