...
La tabla vuelve a estar **vacía**: GEO pasó a `active/` el 2026-08-23 al arrancar su implementación. Antes de él ya lo
estaba — LEGALES, el último que quedaba, se escribió, se implementó y se cerró el mismo día (2026-08-21), así que pasó de acá
a `done/` sin escala real en `active/`.
...
| [LEGALES](done/LEGALES.md) | Ítem 11 — la letra chica: `/legales` pasa a índice y la atribución baja a `/legales/atribucion` (**sin un solo redirect**), más T&C, privacidad y la página de baja de la Resolución 424/2020. El criterio que reemplaza al abogado: **cada cláusula señala una fila del inventario del código**. ⚠️ Lleva una **brecha declarada**: el titular es el dominio + el mail, sin nombre ni CUIT. [Resumen](../archive/SPECS_ARCHIVO.md#legales) · ✅ 2026-08-21 |
