...
| LEG-16 | Los terceros de los dos grupos, con la IP dicha | **PASS** (con enmienda) | Grupo servidor: Anthropic, Mercado Pago, el proveedor de envío de mails y Google Maps Platform. Grupo navegador: Google, OpenFreeMap, el proveedor de almacenamiento, Vercel y el proveedor de base de datos, con *«les pega tu navegador directamente, así que ven tu IP»*. ⚠️ **Enmienda del 2026-08-21, decisión de Fer al revisar la F2 en pantalla**: Resend, Neon y R2 van **por función y no por marca** — son los tres cuyo nombre no se puede averiguar desde afuera. Quedan nombrados Google y OpenFreeMap (visibles en la pestaña Red) y Vercel (`curl -I` sobre producción devuelve `Server: Vercel`). La sustancia del bloque D queda entera y **el documento ofrece el nombre a pedido** por el canal de los derechos. ⚠️ **R2 se corrigió el mismo día**: en la primera pasada se lo dio por público porque en **dev** las fotos salen de `pub-….r2.dev`, pero en **producción** viven detrás de `fotos.adondesalimos.com.ar` — para decidir qué se ve desde afuera hay que mirar prod, no el `.env` de dev |
